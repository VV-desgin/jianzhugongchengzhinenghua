...
_REUSE_YES = {
    "YES", "Y", "OUI", "1", "TRUE", "利旧", "REUSE",
    "REUTILISE", "REUTILISATION",
}
_REUSE_NO = {
    "NO", "N", "NON", "0", "FALSE", "否", "新建", "新购",
}
_REUSE_UNKNOWN = {
    "UNKNOWN", "未知", "待确认", "UNDEFINED", "NULL", "N/A", "NA",
    "不确定", "无法确认",
}


def _reuse_state(obj: dict, params: dict) -> str:
    """利旧状态三态归一：yes / no / unknown。

    yes=确认利旧；no=确认新购/无利旧标记；unknown=利旧状态未知，
    不冲减新购且必须进入人工确认，避免擅自少采购。
    """
    flag = params.get("reuse", {}).get("flag_field", "reuse")
    v = _obj_field(obj, flag, "reuse", "statut", "STATUT", "status")
    s = v.upper()
    if s in _REUSE_YES:
        return "yes"
    if s in _REUSE_UNKNOWN:
        return "unknown"
    if s in _REUSE_NO:
        return "no"
    # 无标记/空值按“新购”处理（安全侧），不当作 unknown 阻塞正常工程
    return "no"


def _is_reuse(obj: dict, params: dict) -> bool:
    """兼容旧调用：只有确认 yes 才视为利旧。"""
    return _reuse_state(obj, params) == "yes"
...
def build_bom(engineering_data: dict, params: dict = None) -> dict:
    ...
    cable_confirm = (
        "待人工确认"
        if (zero_len or unknown_reuse_cables)
        else "自动匹配"
    )
    ...
    if unknown_reuse_cables:
        cable_note += (
            f"；{len(unknown_reuse_cables)}条光缆利旧状态未知（UNKNOWN），"
            f"按新购计（{unknown_reuse_km:.2f}KM），需人工确认是否可利旧"
        )
    ...
    for b in boites:
        ...
        reuse_state = _reuse_state(b, params)
        if reuse_state == "yes":
            box_reused[m] = box_reused.get(m, 0) + 1
        elif reuse_state == "unknown":
            box_unknown[m] = box_unknown.get(m, 0) + 1
    for m, total in box_groups.items():
        reused = box_reused.get(m, 0)
        unknown_count = box_unknown.get(m, 0)
        new_qty = max(0, total - reused)
        note = f"设计{total}个" + (f"，利旧冲减{reused}个" if reused else "")
        if unknown_count:
            note += (
                f"，{unknown_count}个利旧状态未知（UNKNOWN），按新购计，"
                "需人工确认是否可利旧"
            )
        confirm = (
            "自动匹配"
