...
def read_bare_shapefile(path: Path) -> list:
    """直接读取裸 SHP 文件（无 QGIS 工程元数据），供无 QGS/Excel 的纯 SHP 包使用。"""
    from .qgs_reader import QgsLayerMeta
    meta = QgsLayerMeta(name=path.stem, data_source=path.name,
                        geometry_type="Unknown", srs_authid=None)
    return LayerReader(path.parent).read_vector_layer(meta)
