...
SEVERITY_MAP = {
    ...
    "R003": "fatal",  # R-FILE-002~004 图层命名与类型
    "R004": "fatal",  # R-FILE-002~004 图层命名与类型
    "R007": "fatal",  # R-FLD-002 编码唯一性
    "R010": "fatal",  # R-GIS-006 光缆端点空间重合
    "R013": "fatal",
    "R014": "fatal",
    "R015": "fatal",
    "R016": "fatal",  # R-SYS-002 空图层
    "R017": "fatal",  # R-SYS-001 坐标系一致性
    "R023": "fatal",
    "R033": "fatal",  # R-SYS-002 空图层
    # error（严重/高）：记录但不阻断
    "R005": "error",  # R-FLD-001 必填字段非空
    "R005_1": "error",  # R-REL-001 PM与ZPM孤立性
    "R005_2": "error",  # R-REL-002 PBO与PM主从孤立性
    "R005_3": "error",  # R-REL-003 光缆与PM主从孤立性
    "R005_4": "error",  # R-REL-004 光缆端点正向孤立
    "R006": "error",
    "R006_3": "error",  # R-GIS-002/003/004 归属范围包含
    "R006_4": "error",
    "R006_5": "error",
    "R006_6": "error",
    "R007_1": "error",  # R-DAT-001 PBO设计覆盖容量溢出
    "R007_2": "error",  # R-DAT-002 PM汇聚端口总容量
    "R008": "error",
    "R011": "error",
    "R012": "error",
    "R019": "error",  # R-DAT-002 PM汇聚容量
    "R020": "error",  # R-DAT-001 PBO容量
    "R021": "error",  # R-FLD-001 必填字段非空
    "R022": "error",  # R-DAT-001 PBO设计覆盖容量
    "R-FIBER-001": "error",
    "R-BOM-001": "error",  # 附表2 物料无法匹配（2026-08-22 按确认口径，与 R-FIBER-001 一致）
}
...
def _is_root_code(dev_code: str) -> bool:
    """POP/SRO 等根节点（汇聚点/局端）不要求存在于设备图层（口径与标准案例一致）。"""
    try:
        from .code_format import is_root_code
        return is_root_code(dev_code)
    except Exception:
        up = str(dev_code or "").upper()
        return up == "POP" or up.startswith(("POP", "SRO"))
...
def check_reference_exists(ctx: RuleContext) -> List[CheckResult]:
    ...
    for cable in ctx.cables:
        ...
        for field, label in [('start', start), ('end', end)]:
            ...
            if dev_code and dev_code not in device_codes and not _is_root_code(dev_code):
                results.append(CheckResult(
                    check_object=f"光缆 {code}",
                    passed=False,
                    problem_location=f"字段 {field}",
...
def _load_official_material_codes() -> set:
    """从交付固定路径加载官方 BOM_LIST 第一列（官方 29 码）；文件缺失或异常时返回空集，由内置码兜底。"""
    try:
...
DEFAULT_MATERIAL_CODES = {
    ...
    "500002337", "500002159", "500004729", "200000185", "200001033", "200000149",
    "200000148", "200000172", "200000145", "500001103", "200000690", "500002107",
    "500001519", "500002108", "500002108-1", "500001887", "500001853",
}
    ...
    for root in _project_roots(ctx):
        for f in root.rglob("*"):
            ...
            try:
                ...
                continue
            is_bom_list = "bom_list" in f.name.lower()
            if is_bom_list:
                wanted = names  # BOM_LIST.xlsx 作为被检查表，全 Sheet 参与
            else:
                wanted = [n for n in names if "BOM" in n.upper() and "物料" in n]
            if not wanted:
                continue
            for n in wanted:
                try:
...
def check_bom_material_match(ctx: RuleContext) -> List[CheckResult]:
    """R-BOM-001：BOM物料表中物料编码不在已知物料编码库（material_code_*）。

    未找到物料编码库时使用官方默认物料库（BOM_LIST 官方 29 码）兜底，避免漏检。
    """
    results: List[CheckResult] = []
    layer_name = next((k for k in ctx.layers if "BOM" in k.upper() and ("物料" in k or "BOM" == k.upper())), None)
