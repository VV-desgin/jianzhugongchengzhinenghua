...
class ProjectData:
    ...
    def __init__(self, archive_path: str):
        ...
        if not qgs_files:
            ...
            if not self.layers:
                try:
                    from .layer_reader import read_bare_shapefile
                    shp_layers = {}
                    for shp in sorted(self.outer_package.temp_dir.rglob('*.shp')):
                        feats = read_bare_shapefile(shp)
                        if feats:
                            shp_layers.setdefault(shp.stem.upper(), []).extend(feats)
                    if shp_layers:
                        self.layers = shp_layers
                        self.is_bare_shp = True
                        logger.info(f"裸 SHP 工程包识别: {len(shp_layers)} 个图层")
                except Exception as e:
                    logger.warning(f"裸 SHP 加载失败: {e}")
            if not self.layers:
