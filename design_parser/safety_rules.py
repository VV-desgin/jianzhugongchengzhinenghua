...
def _count_mode(proj, keywords) -> int:
    _, cables = _layer(proj, "CABLE")
    hit = 0
    for cable in cables:
        mode = (_prop(cable, "MODE_POSE", "MODE_POSE") or "").upper()
        if any(k.upper() in mode for k in keywords):
            hit += 1
    return hit


def _count_layer_feats(proj, keywords) -> int:
    total = 0
    for key, feats in proj.layers.items():
        upper = key.upper()
        if any(k.upper() in upper for k in keywords):
...
def _safety_rule_execution(proj, cfg: dict) -> Dict[str, dict]:
    """按规则给出“是否执行 + 检查对象实例数”，供 P1-07 四口径统计。"""
    kw = cfg.get("utility_layer_keywords", {})
    aerial = _count_mode(proj, ("AERIEN", "架空", "AERIAL"))
    wall = _count_mode(proj, ("MUR", "WALL", "墙壁", "壁挂"))
    underground = _count_mode(proj, ("SOUTERRAIN", "地下", "直埋", "DUCT", "PIPE"))
    ptech_count = len(_layer(proj, "PTECH")[1])
    power = _count_layer_feats(proj, kw.get("电力线", ["电力", "POWER", "ELEC"]))

    util_counts = {}
    for label in ("电力线", "给水管", "压缩空气管", "热力管", "避雷线接地引线", "工作保护地线"):
        util_counts[label] = _count_layer_feats(
            proj, kw.get(label, [label])
        )
    ...
    return {
        "R-SAFE-001": entry("R-SAFE-001", wall, "无墙壁/壁挂光缆"),
        "R-SAFE-002": entry("R-SAFE-002", aerial, "无架空光缆"),
        "R-SAFE-003": entry(
            "R-SAFE-003", aerial if power else 0,
            "无架空光缆或电力线图层",
        ),
        "R-SAFE-004": entry(
            "R-SAFE-004", util_counts["电力线"] if wall else 0,
            "无墙壁光缆或电力线图层",
        ),
        "R-SAFE-005": entry(
            "R-SAFE-005", util_counts["给水管"] if wall else 0,
            "无墙壁光缆或给水管图层",
        ),
        "R-SAFE-006": entry(
            "R-SAFE-006", util_counts["压缩空气管"] if wall else 0,
            "无墙壁光缆或压缩空气管图层",
        ),
        "R-SAFE-007": entry(
            "R-SAFE-007", util_counts["热力管"] if wall else 0,
            "无墙壁光缆或热力管图层",
        ),
        "R-SAFE-008": entry(
            "R-SAFE-008", util_counts["避雷线接地引线"] if wall else 0,
            "无墙壁光缆或避雷线图层",
        ),
        "R-SAFE-009": entry(
            "R-SAFE-009", util_counts["工作保护地线"] if wall else 0,
            "无墙壁光缆或工作地线图层",
        ),
        "R-SAFE-010": entry(
            "R-SAFE-010", underground,
            "无地下/直埋光缆",
        ),
        "R-SAFE-011": entry(
            "R-SAFE-011", ptech_count if any(util_counts.values()) else 0,
            "无电杆或其他设施图层",
        ),
        "R-SAFE-012": entry(
            "R-SAFE-012", aerial,
...
def run_safety_checks(proj, config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "rule_execution": _safety_rule_execution(proj, cfg),
    }
