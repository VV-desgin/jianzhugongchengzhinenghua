...
- `review.issues` 新增 `problem_category`（官方五大问题分类：数据完整性/空间与安全/资源/逻辑一致性/工程合理性）、`problem_category_label`、`object_ref`（关联 `engineering_data.id`，如 `cable:CDI-...`）、`passed`、`actual_value`、`expected_value`；`review.categories` 按五大类汇总告警数。
