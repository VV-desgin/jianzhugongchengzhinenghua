...
OFFICIAL_RULES = [
    ...
    ("4.16", "图层字段检查", "SITE CODE 不能重名",
     ["R007", "R-FLD-002"], "complete"),
    ("5.1", "孤立性检查", "SITE(PM) 与 ZPM 双向孤立性（SITE.CODE=ZPM.CODE）",
     ["R-GIS-002", "R027", "R-REL-001"], "partial"),
    ("5.2", "孤立性检查", "SITE(PM) 与 BOITE(PBO) 主从双向孤立性（BOITE.REF_PM=SITE.CODE）",
     ["R-GIS-003", "R028", "R-REL-001"], "partial"),
    ("5.3", "孤立性检查", "SITE(PM) 与 CABLE(DISTRIBUTION) 主从双向孤立性（CABLE.REF_PM=SITE.CODE）",
     ["R-GIS-004", "R024", "R-REL-004"], "partial"),
...
def main():
    ...
    lines.append("")
    lines.append(f"生成日期：2026-08-07（工具：`tools/gen_evaluation_report.py`，不依赖 pytest）")
    lines.append("")
