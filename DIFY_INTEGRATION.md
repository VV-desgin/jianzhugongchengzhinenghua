...
- `review.issues[]` 每项包含 `rule_id`、`object_type`、`object_id`、`object_ref`、`passed`、`actual_value`、`expected_value`、`field`、`severity`、`message`、`source`、`problem_category`、`problem_category_label`。
- 异常时返回结构化 `errors`，不会输出 Traceback。
- `project_type` 枚举：`survey_design`、`full_design`、`as_built`、`unknown`（纯 Excel 工程包按 full_design 处理）。
- 纯 Excel 工程包（含官方图层 Sheet 的 xlsx/xls）可直接上传：`review_scope=non_spatial`，返回 `skipped_gis_rules` 清单，GIS 空间规则跳过、字段/CODE/关系/容量规则照常执行。
...
- 项目临时文件保留 2 小时（TTL），`raw-file`/`table-data` 等取数接口需在存活期内调用。
