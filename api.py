...
RULE_ROUTING = {
    "完整设计图": [
        "R-GIS-007","R-FIBER-003","R-LIFE-001","R034",
        "R006","R006_3","R006_4","R006_5","R006_6","R024","R025","R026","R029","R030","R031",
        ...
        "R005_1","R005_2","R005_3","R027","R028","R033"
    ],
    "场勘设计图": [
        "R006","R031",
...
# P1-07：官方《图层表字段说明和数据校验规则.xlsx》可计算需求 39 项
# （口径来源：docs/01 官方规则覆盖矩阵与 EVALUATION_REPORT 第一节）
OFFICIAL_REQUIREMENT_COUNT = 39
SPATIAL_RULE_PREFIXES = ("R-GIS-", "R-SAFE-")


def _review_scope_stats(
    file_category: str,
    layer_names: list,
    executed_rule_ids,
    executed_instances: int,
    spatial_modules_executed: bool,
    spatial_skip_reason: str,
    spatial_rule_execution: Optional[dict] = None,
) -> dict:
    """审查范围四口径统计：需求项/注册函数/适用路由/执行实例。

    与旧的 rule_count 混合口径（路由数 + 固定 18）分离；空间规则是否执行
    由模块调用与跳过原因说明，不再用 bool(layers) 直接当成已证明执行。
    """
    registered = sorted(ALL_RULES.keys())
    routed = sorted(RULE_ROUTING.get(file_category, []))
    applicable = [rid for rid in routed if rid in ALL_RULES]
    spatial_candidates = [
        rid for rid in registered if rid.startswith(SPATIAL_RULE_PREFIXES)
    ]
    executed = sorted(set(executed_rule_ids))
    skipped = [rid for rid in applicable if rid not in executed]
    return {
        "requirement_count": OFFICIAL_REQUIREMENT_COUNT,
        "registered_rule_count": len(registered),
        "applicable_rule_count": len(applicable),
        "executed_rule_count": len(executed),
        "executed_instance_count": int(executed_instances),
        "skipped_rules": skipped,
        "spatial_candidate_rules": spatial_candidates,
        "spatial_modules_executed": bool(spatial_modules_executed),
        "spatial_skip_reason": spatial_skip_reason or "",
        "spatial_rule_execution": spatial_rule_execution or {},
    }


def resolve_object_ref(texts, code_lookup: dict, fallback: str = "") -> str:
    """P2-01：按对象编码词边界匹配，避免 C-1 命中 C-10/C-100。"""
    if fallback and fallback in set(code_lookup.values()):
        return fallback
    haystack = " ".join(str(x or "") for x in texts).upper()
    for code in sorted(code_lookup, key=len, reverse=True):
        pattern = (
            r"(?<![A-Z0-9_\-])"
            + re.escape(code.upper())
            + r"(?![A-Z0-9_\-])"
        )
...
def data_pipeline(
    ...
    try:
        ...
        if rule_ids and has_gis and file_info.get("file_category", "") in reviewable_categories:
            ...
            spatial_modules_executed = False
            spatial_skip_reason = ""
            spatial_rule_execution = {}
            try:
                module_error = ""
                for rid in rule_ids:
                    ...
                    if rid == "R002":
                        ...
                        # 只要存在 BOITE 图层就不做强制的全量 8 图层 fatal，避免误报
                    ...
                    try:
                        ...
                        if isinstance(res, list):
                            ...
                            executed_instance_count += len(res)
                        else:
                            all_results.append(res)
                            executed_instance_count += 1
                    except Exception as e:
                        executed_instance_count += 1
                ...
                if has_gis:
                    try:
                        ...
                        gis_out = run_gis_checks(proj)
                        safe_out = run_safety_checks(proj)
                        spatial_modules_executed = True
                        spatial_skip_reason = ""
                        for _mod_out in (gis_out, safe_out):
                            spatial_rule_execution.update(
                                _mod_out.get("rule_execution") or {}
                            )
                        for _rid, _info in spatial_rule_execution.items():
                            if _info.get("executed"):
                                executed_rule_ids.add(_rid)
                                executed_instance_count += int(_info.get("instances") or 0)
                        for mod_issues in (gis_out["issues"], safe_out["issues"]):
                            for iss in mod_issues:
                                executed_rule_ids.add(str(iss.get("rule_id") or ""))
                                all_results.append(CheckResult(
                ...
                for r in all_results:
                    if not r.passed:
                        obj_ref = resolve_object_ref(
                            (r.check_object, r.error_description, r.problem_location),
                            code_lookup,
                        )
                ...
                result["review"] = {
                    ...
                    "warning_rules": warning_rules,
                    "rule_count": len(executed_rule_ids),
                    ...
                    "reason": "",
                    "error": "",
                    "scope_stats": _review_scope_stats(
                        file_info.get("file_category", ""),
                        list(getattr(proj, "layers", [])),
                        executed_rule_ids,
                        executed_instance_count,
                        spatial_modules_executed,
                        spatial_skip_reason,
                        spatial_rule_execution,
                    ),
                }
