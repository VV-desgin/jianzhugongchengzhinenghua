...
def test_data_pipeline_contract(client, upload_survey):
    ...
    assert set(data["review"].keys()) == {"total_rules", "passed_rules", "failed_rules", "warning_rules", "issues", "categories", "rule_count", "status", "executed", "reason", "error", "scope_stats"}
    scope = data["review"]["scope_stats"]
    assert scope["requirement_count"] == 39
    assert scope["registered_rule_count"] >= scope["applicable_rule_count"] >= scope["executed_rule_count"]
    assert isinstance(scope["executed_instance_count"], int)
    assert isinstance(scope["skipped_rules"], list)
    assert isinstance(scope["spatial_candidate_rules"], list)
    assert isinstance(scope["spatial_rule_execution"], dict)
