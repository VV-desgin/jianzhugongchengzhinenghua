...
FIXED_DATA_DIR = Path(
    os.environ.get(
        "DESIGN_PARSER_FIXED_DATA_DIR",
        Path(__file__).resolve().parents[1] / "docs" / "官方固定数据",
    )
)
HAS_FIXED_DATA = (
    (FIXED_DATA_DIR / "施工规程知识库v2.0.xlsx").is_file()
    and (FIXED_DATA_DIR / "设计对象-物料-工序映射表.xlsx").is_file()
)
requires_fixed_data = pytest.mark.skipif(
    not HAS_FIXED_DATA,
    reason="官方固定数据未随后端仓库提供；部署环境请设置 DESIGN_PARSER_FIXED_DATA_DIR",
)
